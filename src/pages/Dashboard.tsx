    ...
    return(
        <div className={"h-dvh bg-[#202020]"}>
            <div className={"p-2 text-white bg-[#202020] flex outline outline-[#3c4147] rounded-md gap-5 justify-around w-full"}>
                ...
                <div className={`${cardStyle}`}>
                    <h2>Receita mensal: <span>R${monthIncome(jsonStoredTransactions)} <CalcPercent expense={false} currentValue={monthIncome(jsonStoredTransactions)} lastMonthValue={lastMonthIncome(jsonStoredTransactions)} />{}</span></h2>
                    <h3>Receita do mês anterior: <span>R${lastMonthIncome(jsonStoredTransactions)}</span></h3>
                </div>
                <div className={`${cardStyle}`}>
                    <h2>Despesa mensal: <span>R${monthExpense(jsonStoredTransactions)} <CalcPercent expense={true} currentValue={monthExpense(jsonStoredTransactions)} lastMonthValue={lastMonthExpense(jsonStoredTransactions)}/></span></h2>
                    <h3>Despesa do mês anterior: <span>R${lastMonthExpense(jsonStoredTransactions)}</span></h3>
                </div>
                <div className={`${cardStyle}`}>
                    <h2>Balanço mensal: <span>R${monthBal(jsonStoredTransactions)} <CalcPercent expense={false} currentValue={monthBal(jsonStoredTransactions)} lastMonthValue={lastMonthBal(jsonStoredTransactions)} /></span></h2>
